...
///
/// 子进程 stdout 会被管道化并逐行转发到 `maa-agent-output` 通道（复用 Agent 输出
/// 管线），从而显示在 MXU 运行日志中。Windows 下隐藏子进程控制台窗口。
fn mxu_exec_task_action_impl(
    _ctx: &maa_framework::context::Context,
    args: &maa_framework::custom::ActionArgs,
    ...
    // 覆盖 stdout 为管道，用于把 go-service 的用户日志转发到运行日志。
    // stderr 保持 null：go-service 已将自身 stderr 重定向到文件。
    cmd.stdout(std::process::Stdio::piped());

    // Windows 下隐藏子进程控制台窗口（go-service 是控制台子系统程序）。
    ...
    let mut child = match cmd.spawn() {
        Ok(c) => c,
        Err(e) => {
            log::error!("[MXU_EXEC_TASK] Failed to spawn program: {}", e);
            return false;
        }
    };

    // 单独线程按行读取 stdout，转发到 maa-agent-output 通道（复用 Agent 输出管线，
    // 自带批处理与前端刷屏抑制）。
    if let Some(stdout) = child.stdout.take() {
        let batcher = crate::commands::maa_agent::AgentOutputBatcher::new(
            app_handle.clone(),
            ...
            loop {
                ...
                match reader.read_until(b'\n', &mut buffer) {
                    ...
                    Ok(_) => {
                        ...
                        batcher.enqueue("stdout", line.trim_end());
                    }
                    Err(_) => break,
                }
            }
    ...
    match child.wait() {
        ...
        Err(e) => {
            log::error!("[MXU_EXEC_TASK] Failed to wait for program: {}", e);
            false
        }
    }
}
...
pub fn register_all_mxu_actions(
    ...
    // MXU_EXEC_TASK 需要 app_handle + instance_id 才能把子进程 stdout 转发到运行日志，
    // 因此与 killproc 一样单独注册（无法走上面的 reg_action! 宏）。
    let exec_task_app_handle = app_handle.clone();
    let exec_task_instance_id = instance_id.to_string();
    let exec_task_wrapper = move |ctx: &maa_framework::context::Context,
                                  args: &maa_framework::custom::ActionArgs|
          -> bool {
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            mxu_exec_task_action_impl(ctx, args, &exec_task_app_handle, &exec_task_instance_id)
        }))
        .unwrap_or_else(|e| {
            let msg = if let Some(s) = e.downcast_ref::<&str>() {
                s.to_string()
    ...
    if let Err(e) = resource.register_custom_action(
        MXU_EXEC_TASK_ACTION,
        Box::new(FnAction::new(exec_task_wrapper)),
    ) {
        warn!("[MXU] Failed to register {}: {:?}", MXU_EXEC_TASK_ACTION, e);
        failed_count += 1;
    } else {
        info!(
